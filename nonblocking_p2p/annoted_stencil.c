...
int main(int argc, char **argv)
{
    ...
    // NOTE: x+ is in the EAST direction, y+ is in the SOUTH direction!!
    // north,south,east,west are each the RANK of the neighbor! If none, then MPI_PROC_NULL
    north = (ry - 1) * px + rx;
    if (ry - 1 < 0)
        north = MPI_PROC_NULL;
    south = (ry + 1) * px + rx;
    ...
    //Represent data at each point in an array INCLUDING HALOS ON BOTH SIDES!! [data_x1y1, data_x2y2, ...]
    // Note that malloc allocates a given MEMORY SIZE IN BYTES!

    // BELOW: Here buffers for communications are allocated
    /* From alloc_comm_bufs:
    sbufnorth = (double *) malloc(bx * sizeof(double));
    sbufsouth = (double *) malloc(bx * sizeof(double));
    sbufeast = (double *) malloc(by * sizeof(double));
    sbufwest = (double *) malloc(by * sizeof(double));
    rbufnorth = (double *) malloc(bx * sizeof(double));
    ...
    */
    ...
    for (i = 0; i < by; ++i)
        ...
        */
        ...
        // SEND THE DATA JUST TO THE NEIGHBORS
